...
def main():
    ...
    x_test = (linearX_test.to_numpy().T)
    x_test_norm = (x_test - norm_mean)/norm_std
    x_test_full = np.ones((x_test.shape[0]+1,x_test.shape[1]))
    x_test_full[1,:] = x_test_norm[0,:]
    x_test = x_test_full
    # print(prediction(parameters,x))
    np.savetxt('result_1.txt',prediction(parameters,x_test).T,fmt = "%f")
    ...
    plot = ax_c.plot(np.squeeze(learning_history[:,0]),np.squeeze(learning_history[:,1]),np.squeeze(learning_history[:,2]),c = 'green', label = "Learning Path")
    plot[0].set_3d_properties(learning_history[:,2].T)
    surf = ax_c.plot_surface(X, Y, Z,cmap=cm.coolwarm,linewidth=0, antialiased=False, alpha = 0.4)
    ...
    def contourplots(learning_rate,iter):
        ...
        parameters,learning_history = train(learning_rate,min_change)
        # print(parameters)
        plot = ax.plot(np.squeeze(learning_history[:,0]),np.squeeze(learning_history[:,1]),label = "Learning Path",c = "green")
        ...
        fig.savefig(f"contour{iter}.png")
        ax.legend()
        plt.close()
