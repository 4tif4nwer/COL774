...
# python3 dt_mammography.py <train_data_path> <validation_data_path> <test_data_path> <output_folder_path> <question_part>
    ...
    if part == 'a':
        ...
        clfs = GridSearchCV(estimator = tree.DecisionTreeClassifier(random_state=0),param_grid=param_grid)
        ...
        param_grid = [
        {'n_estimators': [100,500,1000], 'max_features' : [1,1.5,2], 'min_samples_split' : [i for i in range(2,30)]}  
        ]

        clfs = GridSearchCV(estimator = RandomForestClassifier(oob_score=True,random_state=0),param_grid=param_grid)
        clfs = clfs.fit(x_train_data, y_train_data)
        clf = clfs.best_estimator_
        output_file.write("Best estimator parameters :\n")
        ...
        for imputation in ['median','mode']:
            ...
            output_file.write("Best estimator parameters :\n")
            output_file.write(f"max_depth = {clf.max_depth}\n")
            output_file.write(f"max_depth = {clf.min_samples_split}\n")
            output_file.write(f"max_depth = {clf.min_samples_leaf}\n\n")
            ...
            clf = clfs.best_estimator_
            output_file.write("Best estimator parameters :\n")
            output_file.write(f"max_depth = {clf.n_estimators}\n")
            output_file.write(f"max_depth = {clf.max_features}\n")
            output_file.write(f"max_depth = {clf.min_samples_split}\n\n")
        ...
        x_train_data,y_train_data,x_val_data,y_val_data,x_test_data,y_test_data = data_loader(train_loc,val_loc,test_loc,'no_filter')
        param_grid = [
